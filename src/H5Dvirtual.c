...
static herr_t H5D__virtual_init(H5F_t *f, H5D_t *dset, bool open_op);
...
H5D__virtual_copy_layout(H5O_layout_t *layout)
{
    ...
    if (virt->tree) {
        ...
        if (NULL == (new_tree = H5RT_copy(virt->tree)))
        ...
        if (virt->not_in_tree_nused > 0) {
            ...
            if (NULL == (virt->not_in_tree_list =
                             H5MM_calloc(virt->not_in_tree_nused * sizeof(H5O_storage_virtual_ent_t *))))
                HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
 ...
 */
static herr_t
H5D__virtual_init(H5F_t *f, H5D_t *dset, bool H5_ATTR_UNUSED open_op)
{
    ...
    if (H5CX_get_vds_view(&storage->view) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get virtual view option");

    /* Get printf gap if view is H5D_VDS_LAST_AVAILABLE, otherwise set to 0 */
    if (storage->view == H5D_VDS_LAST_AVAILABLE) {
        if (H5CX_get_vds_printf_gap(&storage->printf_gap) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get virtual printf gap");
    } /* end if */
    else
        storage->printf_gap = (hsize_t)0;

    /* Get 'use spatial tree' flag */
    if (H5CX_get_vds_use_tree(&storage->use_tree) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get 'use spatial tree' flag");
...
H5D__virtual_read(H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info, H5D_dset_io_info_t *dset_info)
{
    ...
    should_build_tree = H5D__virtual_should_build_tree(storage);
 ...
 */
static herr_t
H5D__virtual_mappings_to_leaves(H5O_storage_virtual_ent_t *mappings, size_t num_mappings,
                                H5RT_leaf_t **leaves_out, H5O_storage_virtual_ent_t ***not_in_tree_out,
                                size_t *leaf_count, size_t *not_in_tree_count, size_t *not_in_tree_nalloc)
{
...
H5D__virtual_should_build_tree(const H5O_storage_virtual_t *storage)
{
    ...
    if (storage->tree || storage->list_nused < H5D_VIRTUAL_TREE_THRESHOLD)
        ...
        /* Get flag from property */
        ret_value = storage->use_tree;
