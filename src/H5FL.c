...
typedef struct H5FL_reg_gc_list_t {
    ...
    struct H5FL_reg_head_t *first;     /* Pointer to the first node in the list of things to garbage collect */
} H5FL_reg_gc_list_t;
...
#ifdef H5FL_DEBUG
            printf("%s: head->name = %s, head->allocated = %d\n", __func__, H5FL_reg_gc_head.first->name, (int)H5FL_reg_gc_head.first->allocated);
#endif /* H5FL_DEBUG */
...
#ifdef H5FL_DEBUG
            printf("%s: head->name = %s, head->allocated = %d\n", __func__,
                   H5FL_blk_gc_head.first->list->name, (int)H5FL_blk_gc_head.first->list->allocated);
#endif /* H5FL_DEBUG */
...
H5FL_fac_init(size_t size)
{
    ...
    H5TS_INIT_GLOBAL(&H5FL_fac_gc_head, H5FL__fac_head_init, H5E_RESOURCE, H5E_CANTINIT, NULL,
                     "can't initialize 'factory' list of lists");
