                     ...
                     const H5FS_section_class_t *cls)
{
    ...
    /* Get # of nodes on list */
    if ((num_nodes = H5SL_count(fspace_node->sect_list)) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTGET, FAIL, "can't get # of nodes on skip list");
    ...
    if (0 == num_nodes) {
        ...
        tmp_fspace_node = H5SL_remove(sinfo->bins[bin].bin_list, &fspace_node->sect_size, false, NULL);
...
H5FS__sect_unlink_size(H5FS_sinfo_t *sinfo, const H5FS_section_class_t *cls, H5FS_section_info_t *sect)
{
    ...
    tmp_sect_node = H5SL_remove(fspace_node->sect_list, &sect->addr, false, NULL);
...
H5FS__sect_unlink_rest(H5FS_t *fspace, const H5FS_section_class_t *cls, H5FS_section_info_t *sect)
{
    ...
    if (!(cls->flags & H5FS_CLS_SEPAR_OBJ)) {
        ...
        tmp_sect_node = H5SL_remove(fspace->sinfo->merge_list, &sect->addr, false, NULL);
        if (tmp_sect_node == NULL || tmp_sect_node != sect)
            HGOTO_ERROR(H5E_FSPACE, H5E_NOTFOUND, FAIL, "can't find section node on size list");
    } /* end if */
...
H5FS__sect_link_size(H5FS_sinfo_t *sinfo, const H5FS_section_class_t *cls, H5FS_section_info_t *sect)
{
    ...
    if (fspace_node == NULL) {
        ...
        if (H5SL_insert(sinfo->bins[bin].bin_list, fspace_node, &fspace_node->sect_size, false) < 0)
    ...
    if (H5SL_insert(fspace_node->sect_list, sect, &sect->addr, false) < 0)
        HGOTO_ERROR(H5E_FSPACE, H5E_CANTINSERT, FAIL, "can't insert free space node into skip list");
    ...
    if (fspace->sinfo->merge_list) {
        do {
            ...
            if (less_sect_node) {
                ...
                if (H5SL_return(less_sect_node) < 0)
                    HGOTO_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL, "can't return skip list node");
            ...
            greater_sect_node = H5SL_above(fspace->sinfo->merge_list, &(*sect)->addr, H5SL_LOCK_SHARED);
            ...
            if (greater_sect_node) {
                ...
                /* Return the skip list node */
                if (H5SL_return(greater_sect_node) < 0)
                    HGOTO_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL, "can't return skip list node");
...
H5FS__sect_find_node(H5FS_t *fspace, hsize_t request, H5FS_section_info_t **node)
{
    ...
    do {
        ...
        if (fspace->sinfo->bins[bin].bin_list) {
            ...
            if (!alignment) { /* no alignment */
                ...
                curr_size_node = H5SL_first(fspace->sinfo->bins[bin].bin_list, H5SL_LOCK_EXCLUSIVE);
                while (curr_size_node != NULL) {
                    ...
                    /* (in case the current node is removed from the skip list) */
    ...
    /* Return any checked out skip list nodes */
    if (curr_size_node && H5SL_return(curr_size_node) < 0)
        HDONE_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL, "can't return skip list node");
    if (curr_sect_node && H5SL_return(curr_sect_node) < 0)
        HDONE_ERROR(H5E_FSPACE, H5E_CANTRELEASE, FAIL, "can't return skip list node");
...
H5FS__sect_assert(const H5FS_t *fspace)
{
    ...
    if (fspace->sinfo->bins) {
        ...
        for (u = 0; u < fspace->sinfo->nbins; u++) {
            ...
            if (fspace->sinfo->bins[u].bin_list) {
                ...
                acc_tot_size_count += (size_t)num_sizes;

                /* Walk through the sections in this bin */
                curr_size_node   = H5SL_first(fspace->sinfo->bins[u].bin_list, H5SL_LOCK_SHARED);
                bin_serial_count = 0;
                bin_ghost_count  = 0;
                while (curr_size_node != NULL) {
                    ...
                    curr_sect_node    = H5SL_first(fspace_node->sect_list, H5SL_LOCK_SHARED);
                    size_serial_count = 0;
                    size_ghost_count  = 0;
                    while (curr_sect_node != NULL) {
    ...
    if (fspace->sinfo->merge_list) {
        ...
        assert(fspace->tot_sect_count == (separate_obj + (size_t)num_sects));
    }
