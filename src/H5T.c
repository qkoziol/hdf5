            ...
            H5_BEFORE_USER_CB(FAIL)
                {
                    status = (H5T_g.soft[i].conv.u.app_func)(src_id, dst_id, &path->cdata, 0, 0, 0, NULL,
                                                             NULL, H5P_PLIST_ID(H5CX_get_dxpl()));
                }
            H5_AFTER_USER_CB(FAIL)
        }
        else
    ...
    if (path->conv.u.app_func) {
        ...
        if (path->conv.is_app) {
            ...
            H5_BEFORE_USER_CB_NOERR(FAIL)
                {
                    status = (path->conv.u.app_func)(conv_ctx->u.free.src_type_id,
                                                     conv_ctx->u.free.dst_type_id, &path->cdata, 0, 0, 0,
                                                     NULL, NULL, H5P_PLIST_ID(H5CX_get_dxpl()));
                }
        ...
        H5_BEFORE_USER_CB(FAIL)
            {
                ret_value = (tpath->conv.u.app_func)(
                    ...
                    buf_stride, bkg_stride, buf, bkg, H5P_PLIST_ID(H5CX_get_dxpl()));
            }
        H5_AFTER_USER_CB(FAIL)
        if (ret_value < 0)
            HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCONVERT, FAIL, "datatype conversion failed");
