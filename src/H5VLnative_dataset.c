                          ...
                          hid_t H5_ATTR_UNUSED dxpl_id, void H5_ATTR_UNUSED **req)
{
    ...
    if (NULL == (dset = H5D__open_name(&loc, name, dapl_plist)))
                         ...
                         void H5_ATTR_UNUSED **req)
{
    ...
    switch (args->op_type) {
        ...
        case H5VL_DATASET_GET_DAPL: {
            H5P_genplist_t *dapl_plist;

            if (NULL == (dapl_plist = H5D_get_access_plist(dset)))
                HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get access property list for dataset");
            args->args.get_dapl.dapl_id = H5P_PLIST_ID(dapl_plist);
