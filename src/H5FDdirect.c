...
H5FD__direct_register(void)
{
    ...
    /* Register the direct driver, if it isn't already */
    if (NULL == H5FD_DIRECT_driver_g)
        if (NULL == (H5FD_DIRECT_driver_g = H5FD__driver_register(&H5FD_direct_g)))
            HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL, "can't register direct driver");

    /* Get ID for direct driver */
    if (H5I_VFL != H5I_get_type(H5FD_DIRECT_id_g)) {
        if ((H5FD_DIRECT_id_g = H5I_register(H5I_VFL, H5FD_DIRECT_driver_g, false)) < 0)
            HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL, "can't create ID for direct driver");

        /* ID is holding a reference to the connector */
        H5FD__driver_inc_rc(H5FD_DIRECT_driver_g);
    }
