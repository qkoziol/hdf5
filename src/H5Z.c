             ...
             void **buf /*in,out*/)
{
    ...
    if (pline && (flags & H5Z_FLAG_REVERSE)) { /* Read */
        for (i = pline->nused; i > 0; --i) {
            ...
            H5E_PAUSE_ERRORS{
                /* Prepare & restore library for user callback */
                H5_BEFORE_USER_CB(FAIL){new_nbytes = (fclass->filter)(tmp_flags, pline->filter[idx].cd_nelmts,
                                                                      pline->filter[idx].cd_values, *nbytes,
                                                                      buf_size, buf);
        }
        H5_AFTER_USER_CB(FAIL)
    }
...
{ /* Write */
    for (idx = 0; idx < pline->nused; idx++) {
        ...
        H5E_PAUSE_ERRORS{
            /* Prepare & restore library for user callback */
            H5_BEFORE_USER_CB(FAIL){new_nbytes = (fclass->filter)(
                                        flags | (pline->filter[idx].flags), pline->filter[idx].cd_nelmts,
