...
H5D__layout_oh_read(H5D_t *dataset)
{
    ...
    if (msg_exists) {
        ...
        if (H5P_set(dataset->shared->dcpl_plist, H5O_CRT_PIPELINE_NAME, &dataset->shared->dcpl_cache.pline) <
            0)
    ...
    if (msg_exists) {
        ...
        if (H5P_set(dataset->shared->dcpl_plist, H5D_CRT_EXT_FILE_LIST_NAME,
                    &dataset->shared->dcpl_cache.efl) < 0)
