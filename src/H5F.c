                       ...
                       void **token_ptr)
{
    ...
    if (H5CX_peek_vol_connector_prop(&connector_prop) < 0)
        HGOTO_ERROR(H5E_FILE, H5E_CANTGET, H5I_INVALID_HID, "can't get VOL connector from API context");
                ...
                unsigned flags, hid_t fcpl_id, hid_t fapl_id, hid_t es_id)
{
    ...
    if (NULL != token) {
        ...
        if (NULL == (es = H5I_object_verify(es_id, H5I_EVENTSET)))
            HGOTO_ERROR(H5E_FILE, H5E_BADTYPE, H5I_INVALID_HID, "not an event set");
               ...
               H5F_scope_t scope, hid_t es_id)
{
    ...
    if (NULL != token) {
        ...
        /* Get event set */
        if (NULL == (es = H5I_object_verify(es_id, H5I_EVENTSET)))
            HGOTO_ERROR(H5E_FILE, H5E_BADTYPE, FAIL, "not an event set");
...
H5Fmount(hid_t loc_id, const char *name, hid_t child_id, hid_t fmpl_id)
{
    ...
    if (NULL == (fmpl = H5P_acquire(fmpl_id, H5P_TYPE_FILE_MOUNT, H5P_LOCK_SHARED, true)))
        HGOTO_ERROR(H5E_FILE, H5E_BADID, FAIL, "can't find object for ID");
    ...
    /* Release resources */
    if (fmpl && H5P_release(fmpl, H5P_LOCK_SHARED) < 0)
        HDONE_ERROR(H5E_FILE, H5E_CANTUNLOCK, FAIL, "unable to unlock property list");
