...
#include "H5FDgds.h"     /* GDS file driver          */
 ...
 *      initialized.
 */
static bool H5FD_gds_init_s = false;

static bool H5FD_GDS_cufile_driver_opened_s = false;

/* static bool reg_once = false; */

/* The driver identification number, initialized at runtime */
hid_t H5FD_GDS_id_g = H5I_INVALID_HID;
...
read_thread_fn(void *data)
{
     ...
     * td->u.rd_devPtr, td->size, td->offset, td->devPtr_offset);
     */

    while (td->size > 0) {
        if (td->size > td->block_size) {
            ret = cuFileRead(td->cfr_handle, td->u.rd_devPtr, td->block_size, td->offset, td->devPtr_offset);
            td->offset += (off_t)td->block_size;
            td->devPtr_offset += (off_t)td->block_size;
            td->size -= td->block_size;
        }
        else {
            ret      = cuFileRead(td->cfr_handle, td->u.rd_devPtr, td->size, td->offset, td->devPtr_offset);
            td->size = 0;
        }
        assert(ret > 0);
    }
...
write_thread_fn(void *data)
{
     ...
     * td->u.wr_devPtr, td->size, td->offset, td->devPtr_offset);
     */
...
/* Prototypes */
static herr_t  H5FD__gds_init(void);
 ...
 */
herr_t
H5FD__gds_register(void)
{
    herr_t ret_value = SUCCEED; /* Return value */

    FUNC_ENTER_PACKAGE

    if (H5I_VFL != H5I_get_type(H5FD_GDS_id_g))
        if ((H5FD_GDS_id_g = H5FD_register(&H5FD_gds_g, sizeof(H5FD_class_t), false)) < 0)
            HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL, "unable to register GDS driver");

done:
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5FD__gds_register() */

/*---------------------------------------------------------------------------
 * Function:    H5FD__gds_unregister
 *
 ...
 */
herr_t
H5FD__gds_unregister(void)
{
    FUNC_ENTER_PACKAGE_NOERR

    /* Reset VFL ID */
    H5FD_GDS_id_g = H5I_INVALID_HID;

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5FD__gds_unregister() */

/*-------------------------------------------------------------------------
 * Function:    H5FD__gds_init
 *
...
H5FD__gds_init(void)
{
    ...
    if (!H5FD_GDS_cufile_driver_opened_s) {
        ...
        if (status.err != CU_FILE_SUCCESS)
            GDS_GOTO_ERROR(FAIL, "unable to open cufile driver", status);
        H5FD_GDS_cufile_driver_opened_s = true;
    }

    /* Indicate that driver is set up */
    H5FD_gds_init_s = true;
...
H5Pset_fapl_gds(hid_t fapl_id, size_t boundary, size_t block_size, size_t cbuf_size)
{
    ...
    if (fapl_id == H5P_DEFAULT)
        HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "can't set values in default property list");
    ...
    if (H5P_set_driver(fapl, H5FD_GDS, &fa, NULL) < 0)
        HGOTO_ERROR(H5E_VFL, H5E_CANTSET, FAIL, "can't set driver configuration info");
                ...
                size_t *cbuf_size /*out*/)
{
    ...
    if (H5FD_GDS != H5P_peek_driver(fapl))
...
if (fd2 <= 2)
    ...
    if (posix_fadvise(fd, 0, 0, POSIX_FADV_RANDOM) != 0)
        HSYS_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL, "can't set POSIX_FADV_RANDOM");

    if (posix_fadvise(fd, 0, 0, POSIX_FADV_DONTNEED) != 0)
        HSYS_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL, "can't set POSIX_FADV_DONTNEED");

    if (posix_fadvise(fd, 0, 0, POSIX_FADV_NOREUSE) != 0)
        HSYS_GOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL, "can't set POSIX_FADV_NOREUSE");
#endif /* ADVISE_OS_DISABLE_READ_CACHE */
    ...
    if (status.err != CU_FILE_SUCCESS)
        GDS_GOTO_ERROR(NULL, "unable to register file with cufile driver", status);
    ...
    if (io_block_size_var)
        file->io_block_size = (size_t)atoi(io_block_size_var);
    ...
    if (o_flags & O_CREAT) {
        if (write(file->fd, buf1, sizeof(int)) < 0) {
            ...
            if (-1 == ftruncate(file->fd, (off_t)0))
                HSYS_GOTO_ERROR(H5E_IO, H5E_SEEKERROR, NULL, "unable to extend file properly");
        }
    }
    else {
 ...
 */
static herr_t
H5FD__gds_flush(H5FD_t *_file, hid_t H5_ATTR_UNUSED dxpl_id, hbool_t H5_ATTR_UNUSED closing)
{
