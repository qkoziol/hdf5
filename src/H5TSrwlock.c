...
H5TS_rwlock_init(H5TS_rwlock_t *lock)
{
    ...
    if (H5_UNLIKELY(H5TS_mutex_init(&lock->mutex, H5TS_MUTEX_TYPE_PLAIN)))
        HGOTO_DONE(FAIL);

    /* Initialize the condition variables */
    if (H5_UNLIKELY(H5TS_cond_init(&lock->read_cv)))
        HGOTO_DONE(FAIL);
    if (H5_UNLIKELY(H5TS_cond_init(&lock->write_cv)))
        HGOTO_DONE(FAIL);
...
H5TS_rwlock_destroy(H5TS_rwlock_t *lock)
{
    ...
    if (H5_UNLIKELY(H5TS_mutex_destroy(&lock->mutex)))
        HGOTO_DONE(FAIL);
    if (H5_UNLIKELY(H5TS_cond_destroy(&lock->read_cv)))
        HGOTO_DONE(FAIL);
    if (H5_UNLIKELY(H5TS_cond_destroy(&lock->write_cv)))
        HGOTO_DONE(FAIL);
