...
static herr_t H5D__dset_ioinfo_init(H5D_dset_io_info_t *dset_info, H5D_storage_t *store);
...
H5D__read(size_t count, H5D_dset_io_info_t *dset_info)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (H5D__dset_ioinfo_init(&dset_info[i], &store[i]) < 0)
...
H5D__write(size_t count, H5D_dset_io_info_t *dset_info)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (!dset_info[i].dset->shared->checked_filters) {
            if (H5Z_can_apply(&dset_info[i].dset->shared->layout,
                              &dset_info[i].dset->shared->dcpl_cache.pline,
                              dset_info[i].dset->shared->type_id) < 0)
 ...
 */
static herr_t
H5D__dset_ioinfo_init(H5D_dset_io_info_t *dset_info, H5D_storage_t *store)
{
    FUNC_ENTER_PACKAGE_NOERR
