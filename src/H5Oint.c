 ...
 */
herr_t
H5O_create(H5F_t *f, size_t size_hint, size_t initial_rc, H5O_loc_t *loc /*out*/)
{
    H5O_t *oh        = NULL;
 ...
 */
herr_t
H5O_apply_ohdr(H5F_t *f, H5O_t *oh, size_t size_hint, size_t initial_rc, H5O_loc_t *loc_out)
{
    haddr_t  oh_addr;
    size_t   oh_size;
    ...
    if (H5CX_get_bad_mesg_count(&oh->store_bad_mesg_count) < 0)
    ...
    if (oh->version > H5O_VERSION_1) {
        ...
        if (H5CX_get_attr_max_compact(&oh->max_compact) < 0)
            HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get max. # of compact attributes");
        if (H5CX_get_attr_min_dense(&oh->min_dense) < 0)
