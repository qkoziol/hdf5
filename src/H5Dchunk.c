...
H5D__chunk_init(H5F_t *f, H5D_t *dset, bool open_op)
{
    ...
    if (H5CX_get_rdcc_nslots(&rdcc->nslots) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get data cache number of slots");
    if (rdcc->nslots == H5D_CHUNK_CACHE_NSLOTS_DEFAULT)
        rdcc->nslots = H5F_RDCC_NSLOTS(f);

    if (H5CX_get_rdcc_nbytes(&rdcc->nbytes_max) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get data cache byte size");
    if (rdcc->nbytes_max == H5D_CHUNK_CACHE_NBYTES_DEFAULT)
        rdcc->nbytes_max = H5F_RDCC_NBYTES(f);

    if (H5CX_get_rdcc_w0(&rdcc->w0) < 0)
    ...
    if (!rdcc->nbytes_max || !rdcc->nslots)
        ...
        if (NULL == rdcc->slot)
            HGOTO_ERROR(H5E_DATASET, H5E_NOSPACE, FAIL, "memory allocation failed");
