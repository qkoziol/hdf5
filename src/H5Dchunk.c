...
static herr_t H5D__chunk_init(H5F_t *f, const H5D_t *dset);
 ...
 */
static herr_t
H5D__chunk_init(H5F_t *f, const H5D_t *const dset)
{
    H5D_chk_idx_info_t idx_info;                            /* Chunked index info */
    ...
    if (H5P_get(dset->shared->dapl_plist, H5D_ACS_DATA_CACHE_NUM_SLOTS_NAME, &rdcc->nslots) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get data cache number of slots");
    if (rdcc->nslots == H5D_CHUNK_CACHE_NSLOTS_DEFAULT)
        rdcc->nslots = H5F_RDCC_NSLOTS(f);

    if (H5P_get(dset->shared->dapl_plist, H5D_ACS_DATA_CACHE_BYTE_SIZE_NAME, &rdcc->nbytes_max) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get data cache byte size");
    if (rdcc->nbytes_max == H5D_CHUNK_CACHE_NBYTES_DEFAULT)
        rdcc->nbytes_max = H5F_RDCC_NBYTES(f);

    if (H5P_get(dset->shared->dapl_plist, H5D_ACS_PREEMPT_READ_CHUNKS_NAME, &rdcc->w0) < 0)
...
H5D__chunk_io_init(H5D_io_info_t *io_info, H5D_dset_io_info_t *dinfo)
{
    ...
    if (io_info->use_select_io != H5D_SELECTION_IO_MODE_OFF &&
        ...
        chunk_node = H5D_CHUNK_GET_FIRST_NODE(dinfo, H5SL_LOCK_SHARED);
        while (chunk_node) {
...
H5D__create_piece_file_map_all(H5D_dset_io_info_t *di, H5D_io_info_t *io_info)
{
    ...
    while (sel_points) {
        ...
        if (H5SL_insert(fm->dset_sel_pieces, new_piece_info, &new_piece_info->index, false) < 0) {
...
H5D__create_piece_mem_map_hyper(const H5D_dset_io_info_t *dinfo)
{
    ...
    /* Get # of pieces in list */
    if ((num_pieces = H5SL_count(fm->dset_sel_pieces)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get # of pieces in skip list");

    /* Check for all I/O going to a single chunk */
    if (1 == num_pieces) {
        /* Get the node */
        curr_node = H5SL_first(fm->dset_sel_pieces, H5SL_LOCK_SHARED);
                   ...
                   const hsize_t *coords, void *_opdata)
{
    ...
    if (chunk_index == fm->last_index) {
        ...
        if (NULL == (piece_info = (H5D_piece_info_t *)H5SL_search(fm->dset_sel_pieces, &chunk_index))) {
            ...
            /* Insert the new chunk into the skip list */
            if (H5SL_insert(fm->dset_sel_pieces, piece_info, &piece_info->index, false) < 0) {
...
H5D__chunk_mdio_init(H5D_io_info_t *io_info, H5D_dset_io_info_t *dinfo)
{
    ...
    piece_node = H5D_CHUNK_GET_FIRST_NODE(dinfo, H5SL_LOCK_SHARED);
...
H5D__chunk_read(H5D_io_info_t *io_info, H5D_dset_io_info_t *dset_info)
{
    ...
    if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
        ...
        if (H5D_LAYOUT_CB_PERFORM_IO(io_info)) {
            ...
            if ((snum_chunks = H5D_CHUNK_GET_NODE_COUNT(dset_info)) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get # of chunks");
            num_chunks = (size_t)snum_chunks;
        ...
        /* Iterate through nodes in chunk skip list */
        chunk_node = H5D_CHUNK_GET_FIRST_NODE(dset_info, H5SL_LOCK_SHARED);
        while (chunk_node) {
