...
H5Dcreate1(hid_t loc_id, const char *name, hid_t type_id, hid_t space_id, hid_t dcpl_id)
{
    ...
    /* Create the dataset */
    if (NULL == (dset = H5VL_dataset_create(vol_obj, &loc_params, name, H5P_LST_LINK_CREATE_g, type_id,
                                            space_id, dcpl, H5P_LST_DATASET_ACCESS_g, H5_REQUEST_NULL)))
...
H5Dopen1(hid_t loc_id, const char *name)
{
    ...
    /* Open the dataset */
    if (NULL ==
        (dset = H5VL_dataset_open(vol_obj, &loc_params, name, H5P_LST_DATASET_ACCESS_g, H5_REQUEST_NULL)))
