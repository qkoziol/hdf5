...
const H5P_libclass_t H5P_CLS_AACC[1] = {{
    ...
    &H5P_LST_ATTRIBUTE_ACCESS_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_ACRT[1] = {{
    ...
    &H5P_LST_ATTRIBUTE_CREATE_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_GACC[1] = {{
    ...
    &H5P_LST_GROUP_ACCESS_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_TCRT[1] = {{
    ...
    &H5P_LST_DATATYPE_CREATE_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_TACC[1] = {{
    ...
    &H5P_LST_DATATYPE_ACCESS_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_MCRT[1] = {{
    ...
    &H5P_LST_MAP_CREATE_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_VINI[1] = {{
    ...
    &H5P_LST_VOL_INITIALIZE_g,    /* Pointer to default property list */
...
const H5P_libclass_t H5P_CLS_RACC[1] = {{
    ...
    &H5P_LST_REFERENCE_ACCESS_g,    /* Pointer to default property list */
...
static H5P_genclass_t **H5P_class_list_s[] = {
    NULL,                        /* H5P_TYPE_USER (0) */
    &H5P_CLS_ROOT_g,             /* H5P_TYPE_ROOT (1) */
    &H5P_CLS_OBJECT_CREATE_g,    /* H5P_TYPE_OBJECT_CREATE (2) */
    &H5P_CLS_FILE_CREATE_g,      /* H5P_TYPE_FILE_CREATE (3) */
    &H5P_CLS_FILE_ACCESS_g,      /* H5P_TYPE_FILE_ACCESS (4) */
    &H5P_CLS_DATASET_CREATE_g,   /* H5P_TYPE_DATASET_CREATE (5) */
    &H5P_CLS_DATASET_ACCESS_g,   /* H5P_TYPE_DATASET_ACCESS (6) */
    &H5P_CLS_DATASET_XFER_g,     /* H5P_TYPE_DATASET_XFER (7) */
    &H5P_CLS_FILE_MOUNT_g,       /* H5P_TYPE_FILE_MOUNT (8) */
    &H5P_CLS_GROUP_CREATE_g,     /* H5P_TYPE_GROUP_CREATE (9) */
    &H5P_CLS_GROUP_ACCESS_g,     /* H5P_TYPE_GROUP_ACCESS (10) */
    &H5P_CLS_DATATYPE_CREATE_g,  /* H5P_TYPE_DATATYPE_CREATE (11) */
    &H5P_CLS_DATATYPE_ACCESS_g,  /* H5P_TYPE_DATATYPE_ACCESS (12) */
    &H5P_CLS_STRING_CREATE_g,    /* H5P_TYPE_STRING_CREATE (13) */
    &H5P_CLS_ATTRIBUTE_CREATE_g, /* H5P_TYPE_ATTRIBUTE_CREATE (14) */
    &H5P_CLS_OBJECT_COPY_g,      /* H5P_TYPE_OBJECT_COPY (15) */
    &H5P_CLS_LINK_CREATE_g,      /* H5P_TYPE_LINK_CREATE (16) */
    &H5P_CLS_LINK_ACCESS_g,      /* H5P_TYPE_LINK_ACCESS (17) */
    &H5P_CLS_ATTRIBUTE_ACCESS_g, /* H5P_TYPE_ATTRIBUTE_ACCESS (18) */
    &H5P_CLS_VOL_INITIALIZE_g,   /* H5P_TYPE_VOL_INITIALIZE (19) */
    &H5P_CLS_MAP_CREATE_g,       /* H5P_TYPE_MAP_CREATE (20) */
    &H5P_CLS_MAP_ACCESS_g,       /* H5P_TYPE_MAP_ACCESS (21) */
    &H5P_CLS_REFERENCE_ACCESS_g, /* H5P_TYPE_REFERENCE_ACCESS (22) */
};

/* List of all default property lists in the library */
/* (order here is important and must be the same as the H5P_plist_type_t enum) */
static H5P_genplist_t **H5P_def_plist_list_s[] = {
    ...
    &H5P_LST_LINK_ACCESS_g,      /* H5P_TYPE_LINK_ACCESS (17) */
    &H5P_LST_ATTRIBUTE_ACCESS_g, /* H5P_TYPE_ATTRIBUTE_ACCESS (18) */
    &H5P_LST_VOL_INITIALIZE_g,   /* H5P_TYPE_VOL_INITIALIZE (19) */
    &H5P_LST_MAP_CREATE_g,       /* H5P_TYPE_MAP_CREATE (20) */
    &H5P_LST_MAP_ACCESS_g,       /* H5P_TYPE_MAP_ACCESS (21) */
    &H5P_LST_REFERENCE_ACCESS_g, /* H5P_TYPE_REFERENCE_ACCESS (22) */
};
...
H5P__init_package(void)
{
    ...
    HDcompile_assert(H5P_LOCK_INT_NONE != H5P_LOCK_INT_EXCLUSIVE);
    HDcompile_assert(H5P_LOCK_INT_NONE != H5P_LOCK_INT_SHARED);
#ifdef H5_HAVE_CONCURRENCY
    /* Make certain that the H5P lock enum stays in sync w/H5TS lock enum */
    ...
    do {
        ...
        for (u = 0; u < NELMTS(init_class); u++) {
            ...
            if (*lib_class->class_id == (-1) &&
                ...
                if (lib_class->def_plist_id && *lib_class->def_plist_id == H5I_INVALID_HID) {
        ...
        if ((nclass + nlist) > 0) {
            ...
            if (nlist > 0) {
                ...
                if (H5I_nmembers(H5I_GENPROP_LST) == 0) {
                    ...
                    H5P_LST_OBJECT_CREATE_ID_g    = H5I_INVALID_HID;
 ...
 */
static herr_t
H5P__unlock_list_cb(void *_plist, H5I_lock_mode_t mode)
{
    H5P_genplist_t *plist = (H5P_genplist_t *)_plist; /* Property list to lock */
...
H5P__close_list_cb(void *_plist, void H5_ATTR_UNUSED **request)
{
    ...
    /* Don't need to acquire locks on private lists */
    if (!plist->is_private) {
...
H5P_copy_plist(const H5P_genplist_t *old_plist, bool app_ref)
{
    ...
    if (NULL == (new_plist = H5FL_CALLOC(H5P_genplist_t)))
        HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, NULL, "memory allocation failed");
    ...
    /* Get # of properties on deleted list */
    if ((nprops = H5SL_count(old_plist->del)) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL, "can't get # of deleted items");

    /* Cycle through the deleted properties & copy them into the new list's deleted section */
    if (nprops > 0) {
        curr_node = H5SL_first(old_plist->del, H5SL_LOCK_SHARED);
        while (curr_node) {
            ...
            if (H5SL_insert(new_plist->del, new_name, new_name, false) < 0)
                HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, NULL, "can't insert property into deleted skip list");
    ...
    /* Get # of properties */
    if ((nprops = H5SL_count(old_plist->props)) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL, "can't get # of properties");

    /* Cycle through the properties and copy them also */
    if (nprops > 0) {
        curr_node = H5SL_first(old_plist->props, H5SL_LOCK_SHARED);
        while (curr_node) {
...
--------------------------------------------------------------------------*/
hid_t
H5P_copy_plist_id(const H5P_genplist_t *old_plist, bool app_ref)
{
    H5P_genplist_t *new_plist = NULL;            /* New property list generated from copy */
    ...
    /* Copy property list */
    if (NULL == (new_plist = H5P_copy_plist(old_plist, app_ref)))
...
H5P__add_prop(H5SL_t *slist, H5P_genprop_t *prop)
{
    ...
    if (H5SL_insert(slist, prop, prop->name, false) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTINSERT, FAIL, "can't insert property into skip list");
...
H5P__create(H5P_genclass_t *pclass, bool is_default, bool app_ref)
{
    ...
    /* Get an ID for the property list, for use in the property class's 'close' callback */
    if ((plist->plist_id = H5I_register(H5I_GENPROP_LST, plist, app_ref)) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, NULL, "unable to register property list");
    ...
    while (NULL != tclass) {
        if (NULL != tclass->create_func) {
            ...
            H5_BEFORE_USER_CB(NULL)
                {
                    status = (tclass->create_func)(plist->plist_id, tclass->create_data);
                }
            H5_AFTER_USER_CB(NULL)
            if (status < 0)
                HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, NULL, "Can't initialize property");
    ...
    if (ret_value == NULL) {
        if (plist != NULL) {
            ...
            if (plist->plist_id > 0)
                H5I_remove(plist->plist_id, false);
...
H5P__cmp_class(const H5P_genclass_t *pclass1, const H5P_genclass_t *pclass2)
{
    ...
    tnode1 = H5SL_first(pclass1->props, H5SL_LOCK_SHARED);
    tnode2 = H5SL_first(pclass2->props, H5SL_LOCK_SHARED);
    while (tnode1 || tnode2) {
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    htri_t H5P_isa_type(plist, type)
        const H5P_genplist_t *plist;    IN: Property list to query
        H5P_plist_type_t type;          IN: Property 'type' (i.e. class)
 RETURNS
    Success: true (1) or false (0)
    Failure: negative
 DESCRIPTION
    This routine queries whether a property list is a member of the property
    list class indicated by the 'type' parameter.
...
--------------------------------------------------------------------------*/
htri_t
H5P_isa_type(const H5P_genplist_t *plist, H5P_plist_type_t type)
{
    ...
    /* Get a pointer to the default class of the proper type */
...
H5P__iterate_plist_cb(void *_item, void *_key, void *_udata)
{
    ...
    /* Add property name to 'seen' list */
    if (H5SL_insert(udata->seen, key, key, false) < 0)
...
H5P__del_plist_cb(H5P_genplist_t *plist, const char *name, H5P_genprop_t *prop, void H5_ATTR_UNUSED *_udata)
{
    ...
    /* Insert property name into deleted list */
    if (H5SL_insert(plist->del, del_name, del_name, false) < 0)
...
/*--------------------------------------------------------------------------
 ...
 USAGE
    herr_t H5P__copy_prop_plist(dst_plist, src_plist, name)
        H5P_genplist_t *dst_plist;    IN: Pointer to destination property list
        H5P_genplist_t *src_plist;    IN: Pointer to source property list
...
H5P__close(H5P_genplist_t *plist)
{
    ...
    /* Get # of properties */
    if ((nprops = H5SL_count(plist->props)) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, FAIL, "can't get # of properties");

    /* Walk through the changed properties in the list */
    if (nprops > 0) {
        curr_node = H5SL_first(plist->props, H5SL_LOCK_SHARED);
        while (curr_node != NULL) {
            /* Get pointer to property from node */
            tmp = (H5P_genprop_t *)H5SL_item(curr_node);
    ...
    if ((sndel = H5SL_count(plist->del)) < 0)
...
/*--------------------------------------------------------------------------
 ...
 RETURNS
    Returns non-negative on success, negative on failure.
 DESCRIPTION
        Closes a property list, including removing its ID from the ID manager.
    Basically, a convenience wrapper around H5I_remove (which calls the free
    callback).
...
H5P_dissolve(H5P_genplist_t *plist)
{
    ...
    /* Sanity checks */
    assert(plist);
    assert(!plist->is_default);

    /* Remove the property list's ID from the ID manager */
    /* Close the property list first, removing it's ID from the ID manager */
    if (H5I_remove(plist->plist_id, true) != plist)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTREMOVE, FAIL, "problem removing property from ID manager");
...
/*-------------------------------------------------------------------------
 * Function:       H5P_new_plist_of_type
 *
 * Purpose:        Create a new property list, of a given type
 *
 * Return:	   Success:	Pointer to new property list
 *		   Failure:	NULL
 *
 *-------------------------------------------------------------------------
 */
H5P_genplist_t *
H5P_new_plist_of_type(H5P_plist_type_t type, bool app_ref)
{
    ...
    /* Check arguments */
    if (type == H5P_TYPE_USER)
        HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, NULL, "can't create user property list");
    if (type == H5P_TYPE_ROOT)
        HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, NULL, "shouldn't be creating root class property list");
 ...
 *		property list structure.
 *
 * Return:      Success:        Flag indicating that property list is a default list
 *              Failure:        N/A
 *
 *-------------------------------------------------------------------------
 */
...
H5P_is_default_plist(const H5P_genplist_t *plist)
{
    ...
    FUNC_LEAVE_NOAPI(plist->is_default)
                  ...
                  mode)
{
    ...
    if (!H5P_PLIST_IS_DEFAULT(plist)) {
        ...
        /* Acquire lock on the property list */
        H5TS_dlftt_rwlock_lock(&plist->lock, (H5TS_rwlock_lock_mode_t)mode);
    }
#endif /* H5_HAVE_CONCURRENCY */
                    ...
                    mode)
{
    ...
    if (!H5P_PLIST_IS_DEFAULT(plist)) {
        /* Private lists don't get locked */
        assert(!plist->is_private);

        /* Release lock on the property list */
        H5TS_dlftt_rwlock_unlock(&plist->lock, (H5TS_rwlock_lock_mode_t)mode);
    }
#endif /* H5_HAVE_CONCURRENCY */
...
H5P_acquire(hid_t plist_id, H5P_plist_type_t type, H5P_lock_mode_t mode, bool allow_default)
{
    ...
    if (H5P_DEFAULT == plist_id) {
        ...
        /* Check the property list's class */
        if (H5P_isa_type(plist, type) != true)
            HGOTO_ERROR(H5E_PLIST, H5E_CANTCOMPARE, NULL, "property list is not a member of the class");

        /* Don't allow default property lists, when requested */
        if (!allow_default && H5P_PLIST_IS_DEFAULT(plist))
            HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, NULL, "property list is a default list");
 ...
 */
herr_t
H5P_release(H5P_genplist_t *plist, H5P_lock_mode_t mode)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if (!H5P_PLIST_IS_DEFAULT(plist))
        /* Unlock the property list */
        if (H5I_release(plist, H5I_GENPROP_LST, (H5I_lock_mode_t)mode) < 0)
