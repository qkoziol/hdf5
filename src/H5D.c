...
static hid_t  H5D__create_api_common(hid_t loc_id, const char *name, hid_t type_id, hid_t space_id, H5P_genplist_t *lcpl, H5P_genplist_t *dcpl, H5P_genplist_t *dapl, void **token_ptr, H5VL_object_t **_vol_obj_ptr);
static hid_t  H5D__open_api_common(hid_t loc_id, const char *name, H5P_genplist_t *dapl, void **token_ptr, H5VL_object_t **_vol_obj_ptr);
 ...
 */
static hid_t
H5D__create_api_common(hid_t loc_id, const char *name, hid_t type_id, hid_t space_id, H5P_genplist_t *lcpl, H5P_genplist_t *dcpl, H5P_genplist_t *dapl, void **token_ptr, H5VL_object_t **_vol_obj_ptr)
{
    ...
    /* Create the dataset */
    if (NULL == (dset = H5VL_dataset_create(*vol_obj_ptr, &loc_params, name, lcpl, type_id, space_id, dcpl, dapl, token_ptr)))
    ...
    if (H5I_INVALID_HID == ret_value)
        if (dset && H5VL_dataset_close(*vol_obj_ptr, H5_REQUEST_NULL) < 0)
...
H5Dcreate_anon(hid_t loc_id, hid_t type_id, hid_t space_id, hid_t dcpl_id, hid_t dapl_id)
{
    ...
    /* Create the dataset */
    if (NULL == (dset = H5VL_dataset_create(vol_obj, &loc_params, NULL, H5P_LST_LINK_CREATE_g, type_id, space_id, dcpl, dapl, H5_REQUEST_NULL)))
 ...
 */
static hid_t
H5D__open_api_common(hid_t loc_id, const char *name, H5P_genplist_t *dapl, void **token_ptr, H5VL_object_t **_vol_obj_ptr)
{
    void           *dset = NULL;        /* dset object from VOL connector */
    hid_t           dapl_id;            /* ID for dataset access property list */
    H5VL_object_t  *tmp_vol_obj = NULL; /* Object for loc_id */
    ...
    /* Open the dataset */
    if (NULL == (dset = H5VL_dataset_open(*vol_obj_ptr, &loc_params, name, dapl, token_ptr)))
...
H5D__get_space_api_common(hid_t dset_id, void **token_ptr, H5VL_object_t **_vol_obj_ptr)
{
    ...
    /* Get the dataspace */
    if (H5VL_dataset_get(*vol_obj_ptr, &vol_cb_args, token_ptr) < 0)
...
H5Dget_offset(hid_t dset_id)
{
    ...
    /* Get the offset */
    if (H5VL_dataset_optional(vol_obj, &vol_cb_args, H5P_LST_DATASET_XFER_g, H5_REQUEST_NULL) < 0)
                     ...
                     hid_t dxpl_id, const void *buf[], hid_t es_id)
{
    ...
    /* Write the data */
    if (H5D__write_api_common(count, dset_id, mem_type_id, mem_space_id, file_space_id, dxpl, buf, token_ptr,
                              &vol_obj) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_WRITEERROR, FAIL, "can't asynchronously write data");
 ...
 */
static herr_t
H5D__set_extent_api_common(hid_t dset_id, const hsize_t size[], void **token_ptr, H5VL_object_t **_vol_obj_ptr)
{
    H5VL_object_t  *tmp_vol_obj = NULL; /* Object for loc_id */
    H5VL_object_t **vol_obj_ptr =
        (_vol_obj_ptr ? _vol_obj_ptr : &tmp_vol_obj); /* Ptr to object ptr for loc_id */
    ...
    /* Set the extent */
    if (H5VL_dataset_specific(*vol_obj_ptr, &vol_cb_args, token_ptr) < 0)
...
H5Dflush(hid_t dset_id)
{
     ...
     */
    if (H5VL_dataset_specific(vol_obj, &vol_cb_args, H5_REQUEST_NULL) < 0)
