...
H5Tcommit1(hid_t loc_id, const char *name, hid_t type_id)
{
    ...
    /* Commit the datatype */
    if (NULL == (data = H5VL_datatype_commit(vol_obj, &loc_params, name, type_id, H5P_LINK_CREATE_DEFAULT, def_tcpl, def_tapl, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
...
H5Topen1(hid_t loc_id, const char *name)
{
    ...
    /* Open the datatype */
    if (NULL == (dt = H5VL_datatype_open(vol_obj, &loc_params, name, def_tapl, H5P_DATASET_XFER_DEFAULT, H5_REQUEST_NULL)))
