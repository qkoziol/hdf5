...
H5Tcommit1(hid_t loc_id, const char *name, hid_t type_id)
{
    ...
    /* Commit the datatype */
    if (NULL ==
        (data = H5VL_datatype_commit(vol_obj, &loc_params, name, type_id, H5P_LST_LINK_CREATE_g,
                                     H5P_LST_DATATYPE_CREATE_g, H5P_LST_DATATYPE_ACCESS_g, H5_REQUEST_NULL)))
...
H5Topen1(hid_t loc_id, const char *name)
{
    ...
    /* Open the datatype */
    if (NULL ==
        (dt = H5VL_datatype_open(vol_obj, &loc_params, name, H5P_LST_DATATYPE_ACCESS_g, H5_REQUEST_NULL)))
