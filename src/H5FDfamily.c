...
typedef struct H5FD_family_fapl_t {
    ...
    H5P_genplist_t *memb_fapl; /*file access property list pointer for members    */
} H5FD_family_fapl_t;

/* The description of a file belonging to this driver. */
typedef struct H5FD_family_t {
    H5FD_t             pub;       /*public stuff, must be first        */
    H5FD_family_fapl_t fa;        /*family driver info */
    ...
    H5FD_int_t       **memb;      /*dynamic array of member pointers    */
...
static herr_t  H5FD__family_get_handle(H5FD_t *_file, hid_t fapl_id, void **file_handle);
...
H5FD__family_get_default_config(H5FD_family_fapl_t *fa_out)
{
    ...
    /* Use copy of default file access property list for member FAPL.
     * The sec2 driver is explicitly set on the member FAPL, as the
...
H5FD__family_register(void)
{
    ...
    if (NULL == H5FD_FAMILY_driver_g)
        if (NULL == (H5FD_FAMILY_driver_g = H5FD__driver_register(&H5FD_family_g)))
            HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL, "can't register family driver");

    /* Get ID for family driver */
    if (H5I_VFL != H5I_get_type(H5FD_FAMILY_id_g)) {
        if ((H5FD_FAMILY_id_g = H5I_register(H5I_VFL, H5FD_FAMILY_driver_g, false)) < 0)
            HGOTO_ERROR(H5E_VFL, H5E_CANTREGISTER, FAIL, "can't create ID for family driver");

        /* ID is holding a reference to the connector */
        H5FD__driver_inc_rc(H5FD_FAMILY_driver_g);
    }
...
H5Pset_fapl_family(hid_t fapl_id, hsize_t msize, hid_t memb_fapl_id)
{
    ...
    else if (NULL == (fa.memb_fapl = H5P_acquire(memb_fapl_id, H5P_TYPE_FILE_ACCESS, H5P_LOCK_SHARED, true)))
    ...
    if (is_default) {
        ...
        if (fa.memb_fapl && H5P_release(fa.memb_fapl, H5P_LOCK_SHARED) < 0)
...
H5Pget_fapl_family(hid_t fapl_id, hsize_t *msize /*out*/, hid_t *memb_fapl_id /*out*/)
{
    ...
    if (msize)
        *msize = fa->memb_size;
    if (memb_fapl_id)
        if ((*memb_fapl_id = H5P_copy_plist_id(fa->memb_fapl, true)) < 0)
...
H5Pget_family_offset(hid_t fapl_id, hsize_t *offset /*out*/)
{
    ...
    /* Get the pointer to the property list object */
    if (NULL == (fapl = H5P_acquire(fapl_id, H5P_TYPE_FILE_ACCESS, H5P_LOCK_SHARED, false)))
        HGOTO_ERROR(H5E_ARGS, H5E_BADID, FAIL, "can't find object for ID");
...
H5FD__family_fapl_get(H5FD_t *_file)
{
    ...
    if (NULL == (ret_value = H5FD__family_fapl_copy(&file->fa)))
...
H5FD__family_fapl_copy(const void *_old_fa)
{
    ...
    if (NULL == (new_fa = H5MM_calloc(sizeof(H5FD_family_fapl_t))))
    ...
    if (ret_value == NULL)
        if (new_fa && H5FD__family_fapl_free(new_fa) < 0)
            HDONE_ERROR(H5E_VFL, H5E_CANTCLOSEOBJ, NULL, "can't close family driver info");
...
H5FD__family_sb_decode(H5FD_t *_file, const char H5_ATTR_UNUSED *name, const unsigned char *buf)
{
    ...
    if (file->mem_newsize)
        file->fa.memb_size = file->pmem_size = file->mem_newsize;
    else {
...
H5FD__family_open(const char *name, unsigned flags, hid_t H5_ATTR_UNUSED fapl_id, haddr_t maxaddr)
{
    ...
    if (H5CX_get_family_newsize(&file->mem_newsize) < 0)
        HGOTO_ERROR(H5E_PLIST, H5E_CANTGET, NULL, "can't get new family member size");
    if (file->mem_newsize > H5F_ACS_FAMILY_NEWSIZE_DEF)
        /* Set flag for later */
        file->repart_members = true;

    file->pmem_size = file->fa.memb_size; /* Member size passed in through property */
    ...
    while (1) {
        ...
        if (file->nmembs >= file->amembs) {
            ...
            assert(n > 0);
            if (NULL == (x = H5MM_realloc(file->memb, n * sizeof(H5FD_int_t *))))
     ...
     * member size.
     */
    if (0 != (eof = H5FD_get_eof(file->memb[0], H5FD_MEM_DEFAULT)))
...
H5FD__family_get_handle(H5FD_t *_file, hid_t fapl_id, void **file_handle)
{
    ...
    if (offset > (file->fa.memb_size * file->nmembs))
 ...
 */
static herr_t
H5FD__family_read(H5FD_t *_file, H5FD_mem_t type, hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr, size_t size,
                  void *_buf /*out*/)
{
    H5FD_family_t *file = (H5FD_family_t *)_file;
    ...
    while (size > 0) {
        H5_CHECKED_ASSIGN(u, unsigned, addr / file->fa.memb_size, hsize_t);
 ...
 */
static herr_t
H5FD__family_write(H5FD_t *_file, H5FD_mem_t type, hid_t H5_ATTR_UNUSED dxpl_id, haddr_t addr, size_t size,
                   const void *_buf)
{
    H5FD_family_t       *file = (H5FD_family_t *)_file;
